...
terraform {
  ...
  backend "s3" {
    ...
    key     = "archivematica/staging/terraform.tfstate"
    region  = "us-west-2"
    encrypt = true
  }
}
...
locals {
  current_archivematica_deploy = try(data.kubernetes_resource.archivematica_staging.object, null)
  current_mcp_client_deploy = try(data.kubernetes_resource.mcp_client_staging.object, null)
...
module "archivematica" {
  ...
  storage_service_image = local.desired_images["archivematica-storage-service-staging"]
  dashboard_image       = local.desired_images["archivematica-dashboard-staging"]
  mcp_server_image      = local.desired_images["archivematica-mcp-server-staging"]
  mcp_client_image      = local.desired_images["archivematica-mcp-client-staging"]

  # Service configuration
  gearman_service_name = module.gearman.service_name
  allowed_hosts        = "staging.archivematica.permanent.org"
...
module "ingress" {
  ...
  security_group_id       = var.staging_security_group_id
